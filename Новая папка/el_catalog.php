...
<div class="bx-catalog-element<?=$themeClass?>" id="<?=$itemIds['ID']?>" itemscope itemtype="http://schema.org/Product">
	...
	<div class="row g-xl-5 mb-4">
            							...
            							<div class="product-item-detail-slider-controls-block" id="<?=$itemIds['SLIDER_CONT_OF_ID'].$offer['ID']?>" style="display: <?=$strVisible?>;">
            								...
            								foreach ($offer['MORE_PHOTO'] as $keyPhoto => $photo)
            								{
            									...
            									<div class="product-item-detail-slider-controls-image<?=($keyPhoto == 0 ? ' active' : '')?>"
            										...
            										<img src="<?=$photo['SRC']?>" alt="Превью" title="Превью">
            									</div>
            									<?php
            								}
            								?>
        						...
        						if (!empty($actualItem['MORE_PHOTO']))
        						{
        							foreach ($actualItem['MORE_PHOTO'] as $key => $photo)
        							{
        								...
        								<div class="product-item-detail-slider-image<?=($key == 0 ? ' active' : '')?>" data-entity="image" data-id="<?=$photo['ID']?>">
        								    <a href="<?=$photo['SRC']?>" class="glightbox" data-gallery="product">
		...
		?>
		<div class="<?=($showBlockWithOffersAndProps ? "col-md-5 col-lg-4" : "col-md-4"); ?>">
		    <div class="product-detail-right" style="position: sticky; top: 24px">
		        <?php
						...
						switch ($blockName)
						{
								...
								<div class="mb-3">
									...
									if ($arParams['SHOW_OLD_PRICE'] === 'Y')
									{
										...
										<div class="product-item-detail-price-old mb-1"
											...
											><?= $price['PRINT_RATIO_BASE_PRICE']?></div>
										<?php
									}
									?>
							...
							case 'buttons':
								...
								<div data-entity="main-button-container" class="mb-3">
									<div id="<?=$itemIds['BASKET_ACTIONS_ID']?>" style="display: <?=($actualItem['CAN_BUY'] ? '' : 'none')?>;">
										...
										if ($showAddBtn)
										{
											?>
											<div class="mb-3">
												<a class="button <?=$showButtonClassName?> product-item-detail-buy-button"
										...
										if ($showBuyBtn)
										{
											?>
											<div class="mb-3">
												<a class="button <?=$buyButtonClassName?> product-item-detail-buy-button"
													id="<?=$itemIds['BUY_LINK']?>"
													href="javascript:void(0);">
								...
								<div class="mb-3" id="<?=$itemIds['NOT_AVAILABLE_MESS']?>" style="display: <?=(!$actualItem['CAN_BUY'] ? '' : 'none')?>;">
									<a class="button product-item-detail-buy-button" href="javascript:void(0)" rel="nofollow"><?=$arParams['MESS_NOT_AVAILABLE']?></a>
								</div>
								<?php
								break;
						}
	...
	<div class="p-2 product-item-detail-short-card-fixed d-none" id="<?=$itemIds['SMALL_CARD_PANEL_ID']?>">
		<div class="product-item-detail-short-card-content-container">
			<div class="product-item-detail-short-card-image">
				<img src="" style="height: 65px;" data-entity="panel-picture" alt="<?=$name?>" title="<?=$name?>">
			</div>
